...
/* ================= Database SDK Integration ================= */
  ...
  // v3.17.39 统一 DOM 写入助手：宿主缺失时只告警、不抛错（治理裸写，避免"设置 null 属性"类致命浮层）。
  // 缺失元素按 id 仅告警一次，便于定位但不刷屏。
  const missingWarned=new Set();
  function needEl(id,el){if(!el&&!missingWarned.has(id)){missingWarned.add(id);console.warn("[life] 缺少元素 #"+id+"（写入已跳过）");}return el;}
  function $el(id){return needEl(id,document.getElementById(id));}
  function setText(id,text){const e=$el(id);if(e)e.textContent=text;return !!e;}
  function setHtml(id,html){const e=$el(id);if(e)e.innerHTML=html;return !!e;}
  function setProp(id,prop,value){const e=$el(id);if(e)e[prop]=value;return !!e;}
  function setStyle(id,prop,value){const e=$el(id);if(e)e.style[prop]=value;return !!e;}
  function classOp(id,op,...args){const e=$el(id);if(e&&e.classList&&typeof e.classList[op]==='function')e.classList[op](...args);return !!e;}
  function focusFormField(formId,field){const f=document.getElementById(formId);if(f&&f.elements&&f.elements[field])f.elements[field].focus();}
  ...
  function renderDashboard(){
    ...
    setText('lifeScore',score);
    setText('heroSummary',t(score>=85?'今天的节奏很好，也记得留一点空白。':score>=70?'已经在稳稳推进，继续保持自己的节奏。':'今天先从一件小事开始，慢慢来就好。'));
    setText('monthExpense',money(monthExpense));
    setText('budgetHint',monthExpense>state.settings.budget?'已超出本月预算':`还可安排 ${money(Math.max(0,state.settings.budget-monthExpense))}`);
    setText('habitProgress',`${completed} / ${state.habits.length}`);
    setText('habitHint',completed===state.habits.length?'今天全部完成':'从一件小事开始');
    setText('taskProgress',`${todayTasks.filter(r=>!r.data.done).length} 件`);
    setText('taskHint',doneTasks?`已经完成 ${doneTasks} 件`:'节奏刚刚好');
    setHtml('dashboardTasks',todayTasks.length?todayTasks.slice(0,4).map(r=>taskRow(r)).join(''):empty('今天还没有待办，给自己留点空间'));
    setHtml('dashboardHabits',state.habits.slice(0,5).map(h=>`<div class="habit-pill"><span class="habit-dot ${h.tone}"></span><strong>${habitNameHtml(h)}</strong><small>${t(habitDone(h)?'已完成':'待打卡')}</small><button class="check-btn ${habitDone(h)?'checked':''}" data-action="habit-quick" data-id="${h.id}">${habitDone(h)?icon('i-check'):''}</button></div>`).join(''));
    const recent=sortedRecords().slice(0,5);setHtml('recentRecords',recent.length?recent.map(r=>{const meta=TYPE_META[r.type]||TYPE_META.home;return`<div class="timeline-item"><span class="record-icon ${meta.tone}">${icon(meta.icon)}</span><span><strong>${recordTitleHtml(r)}</strong><small>${escapeHtml(formatDateHeading(r.date))} · ${t(meta.label)}</small></span><span class="record-value">${escapeHtml(valueFor(r))}</span></div>`;}).join(''):empty('第一条记录，会从这里开始'));
    setText('insightText',completed?'你不是在追赶完美，而是在让好习惯慢慢变得自然。':'规律不是把每天塞满，而是知道什么值得留下。');
  }
  ...
  function renderMoney(){
    ...
    if(prevExpense){const diff=expense-prevExpense,pct=Math.abs(diff/prevExpense*100).toFixed(0);setText('monthCompare',`比上月${diff>=0?'多':'少'}花了 ${money(Math.abs(diff))}`);setText('monthCompareDetail',`${diff>=0?'↑':'↓'} ${pct}% · 上月 ${money(prevExpense)}`);}else{setText('monthCompare','暂无对比');setText('monthCompareDetail','有了上月数据后，这里会显示变化');}
    const used=state.settings.budget?Math.round(expense/state.settings.budget*100):0;setProp('budgetInput','value',state.settings.budget);setStyle('budgetBar','width',`${Math.min(100,used)}%`);setText('budgetUsedText',`已使用 ${used}%`);setText('budgetRemainText',`剩余 ${money(state.settings.budget-expense)}`);
    ...
    setHtml('moneyFilter','<option value="all">全部分类</option>'+categories.map(c=>`<option ${state.settings.moneyFilter===c?'selected':''}>${c}</option>`).join(''));
    ...
    if(filtered.length){
      ...
      setHtml('moneyList',Object.keys(groups).sort((a,b)=>b.localeCompare(a)).map(date=>{
      ...
      setHtml('moneyList',empty('这个分类还没有流水'));
    }
  ...
  function renderFitness(){
    ...
    setText('latestWeight',`${Number(s.current).toFixed(1)} kg`);setText('weightRemain',LANG==='en'?`${s.remaining.toFixed(1)} kg`:`${(s.remaining*2).toFixed(1)} 斤`);setText('currentBmi',s.bmi.toFixed(1));
    const progress=s.start===s.target?100:clamp((s.start-s.current)/(s.start-s.target)*100,0,100);setText('fitnessPercent',`${Math.round(progress)}%`);setStyle('fitnessProgressBar','width',`${progress}%`);setText('fitnessStartText',`起点 ${s.start} kg`);setText('fitnessTargetText',`目标 ${s.target} kg`);setText('fitnessEstimate',s.remaining<=0?'目标已达成，进入稳定期':s.days?`按当前趋势，约还需 ${s.days} 天`:'多记录几天后估算达成时间');
    ...
    drawWeightChart(s.records);const donePlan=state.settings.weeklyPlan.filter(x=>x.done).length;setText('planProgress',LANG==='en'?`${donePlan} / ${state.settings.weeklyPlan.length} completed`:`${donePlan} / ${state.settings.weeklyPlan.length} 已完成`);setHtml('weeklyPlan',state.settings.weeklyPlan.length?state.settings.weeklyPlan.map(item=>{const isDefault=DEFAULT_PLAN.some(plan=>plan.id===item.id),title=isDefault?(LANG==='en'&&item.titleEn?item.titleEn:localizedHtml(item.title)):userHtml(item.title),note=isDefault?(LANG==='en'&&item.noteEn?item.noteEn:localizedHtml(item.note)):userHtml(item.note);return`<div class="plan-item ${item.done?'done':''}"><button class="check-btn ${item.done?'checked':''}" data-action="toggle-plan" data-id="${item.id}">${item.done?icon('i-check'):''}</button><span><strong>${title}</strong><small>${t(item.group)} · ${note}</small></span><button class="plan-delete" data-action="delete-plan" data-id="${item.id}" aria-label="${t('删除计划')}">${icon('i-trash')}</button></div>`;}).join(''):empty('还没有周计划，点击右上角新增一项'));
    setHtml('fitnessList',s.records.length?s.records.slice().reverse().slice(0,20).map(recordRow).join(''):empty('记录体重和体脂，关注趋势而不是单日数字'));
  }
  ...
  function renderPlanner(){
    ...
    setText('plannerToday',records.filter(r=>r.date===today&&!r.data.done).length);setText('plannerOverdue',records.filter(r=>r.date<today&&!r.data.done).length);setText('plannerWeek',records.filter(r=>r.date>=today&&r.date<=weekEnd&&!r.data.done).length);
    const weekHead=document.getElementById('plannerWeekHead');if(weekHead)weekHead.textContent=range.label;
    setHtml('weekStrip',range.days.map((date,i)=>{const d=new Date(`${date}T00:00:00`),count=records.filter(r=>r.date===date&&!r.data.done).length,weekdays=LANG==='en'?['Sun','Mon','Tue','Wed','Thu','Fri','Sat']:['日','一','二','三','四','五','六'];const isToday=date===today,isPicked=plannerWeekPicked===date;return`<button type="button" class="week-day ${isToday?'today':''} ${isPicked?'picked':''}" data-planner-day="${date}" aria-label="${date}"><span>${weekdays[d.getDay()]}</span><strong>${d.getDate()}</strong><small>${count?(LANG==='en'?`${count} items`:`${count} 项`):t('留白')}</small></button>`;}).join(''));
  ...
  function renderHome(){
    ...
    setText('homeTotal',pending.length);setText('homeBudget',money(sum(pending,r=>r.data.price)));setText('homeBought',bought.filter(r=>(r.data.boughtDate||r.date).startsWith(month)).length);
  ...
  function renderMedia(){
    ...
    setText('mediaYear',year);setText('mediaFinished',`${finished.length} 部`);setText('mediaAverage',average?`${average.toFixed(1)} ★`:'—');setText('mediaFavorite',favorite);
    const distribution=Array.from({length:5},(_,i)=>rated.filter(item=>item.rating===i+1).length),max=Math.max(1,...distribution);setHtml('ratingDistribution',distribution.map((count,i)=>`<div class="rating-bar"><b>${count}</b><span style="--h:${Math.max(4,count/max*72)}px"></span><small>${i+1} 星</small></div>`).join(''));
    document.querySelectorAll('[data-media-view]').forEach(button=>button.classList.toggle('active',button.dataset.mediaView===state.settings.mediaView));setProp('mediaStatusFilter','value',state.settings.mediaStatusFilter);setProp('mediaRatingFilter','value',String(state.settings.mediaRatingFilter||0));
  ...
  function openHabitSettings(){renderHabitManageList();setProp('habitSettings','hidden',false);setTimeout(()=>focusFormField('habitSettingsForm','name'),80);}
  function closeHabitSettings(){setProp('habitSettings','hidden',true);}
  ...
  function openPlanSettings(){renderPlanManageList();setProp('planSettings','hidden',false);setTimeout(()=>focusFormField('planSettingsForm','title'),80);}
  function closePlanSettings(){setProp('planSettings','hidden',true);}
  function openFitnessProfile(){const form=document.getElementById('fitnessProfileForm'),profile=state.settings.fitnessProfile;['height','target','age','sex','activity'].forEach(key=>form.elements[key].value=profile[key]);setProp('fitnessProfileSettings','hidden',false);setTimeout(()=>form.elements.height.focus(),80);}
  function closeFitnessProfile(){setProp('fitnessProfileSettings','hidden',true);}
  ...
  function closeBrandSettings(){setProp('brandSettings','hidden',true);}
  // v3.17.39 失败隔离：单模块渲染异常不再中断整条渲染链
  function renderAll(){const steps=[applyBrand,renderDashboard,renderMoney,renderHabits,renderFitness,renderPlanner,renderHome,renderMedia,renderArchive,renderBackupStatus];for(const fn of steps){try{fn();}catch(err){console.warn('[life] 渲染失败: '+(fn.name||'?'),err&&err.message);}}}
  ...
  function bindForms(){
    ...
    document.getElementById('habitTypeSelect').addEventListener('change',e=>{const form=document.getElementById('habitSettingsForm'),isCheck=e.target.value==='check';form.elements.target.value=isCheck?'1':form.elements.target.value;form.elements.unit.value=isCheck?'次':form.elements.unit.value;classOp('habitTargetFields','toggle','is-check',isCheck);});
  ...
  function init(){subscribeUpdates();setSyncState('syncing', LANG==='en' ? 'Syncing…' : '同步中…');const now=new Date(),weekdays=['星期日','星期一','星期二','星期三','星期四','星期五','星期六'];setText('todayLabel',LANG==='en'?new Intl.DateTimeFormat('en-US',{month:'long',day:'numeric',weekday:'long'}).format(now):`${now.getMonth()+1} 月 ${now.getDate()} 日 · ${weekdays[now.getDay()]}`);setDateDefaults();updateMoneyCategories();restoreDrafts();for(const fn of [bindForms,bindEvents,initFormPanelToggles]){try{fn();}catch(err){console.warn('[life] 绑定失败: '+(fn.name||'?'),err&&err.message);}}renderAll();switchView(document.getElementById(`view-${location.hash.slice(1)}`)?location.hash.slice(1):'dashboard');if(!dataCorrupted)saveState();pullAllRemote(function(ok){ if(ok){ saveState(); renderAll(); } });}
