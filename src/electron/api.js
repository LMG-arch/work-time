// electron/api.js — Web/Capacitor 兼容 API 层（ESM 模块）
// 行为与原 web-api.js 逐字节一致：以持久化存储（storage.js）为后端的
// window.calendarAPI 实现，覆盖平台差异（Electron 桌面端 localStorage 落在 userData，
// Capacitor 走 Filesystem）。原 Electron preload 注入的 IPC 版 window.calendarAPI
// 在经典脚本期被本实现覆盖——此为既有行为，迁移后保持一致，不引入回归。
import { HOLIDAYS, FIXED_HOLIDAYS } from '../holidays.js'

const STORAGE_KEY = 'work-calendar-data';

function loadStore() {
  try {
    const store = window.__storage.get(STORAGE_KEY);
    if (store && store.days) {
...
function saveStore(store) {
  try {
    window.__storage.set(STORAGE_KEY, store);
  } catch (e) {
...
window.calendarAPI = {
  ...
  async exportData() {
    ...
    let reminders = null;
    let reminderRecords = {};
    try { reminders = window.__storage.get('calendar-reminders'); } catch (e) { console.warn('[Export] Failed to parse reminders:', e.message); }
    try { reminderRecords = window.__storage.get('calendar-reminder-records') || {}; } catch (e) { console.warn('[Export] Failed to parse records:', e.message); }
    const exportObj = { ...store, reminders, reminderRecords };
      ...
      input.onchange = async () => {
        ...
        try {
          ...
          if (Array.isArray(imported.reminders)) {
            window.__storage.set('calendar-reminders', imported.reminders);
          }
          if (imported.reminderRecords && typeof imported.reminderRecords === 'object') {
            window.__storage.set('calendar-reminder-records', imported.reminderRecords);
          }
  ...
  async getReminders() {
    try {
      const raw = window.__storage.getRaw('calendar-reminders');
      if (raw) {
  ...
  async saveReminders(reminders) {
    ...
    window.__storage.set('calendar-reminders', wrapper);
    if (typeof autoSyncPush === 'function') autoSyncPush();
    return { success: true };
  },
  ...
  async confirmReminder(date, reminderId) {
    ...
    let records = {};
    try {
      const raw = window.__storage.getRaw(key);
    ...
    window.__storage.set(key, records);
    if (typeof autoSyncPush === 'function') autoSyncPush();
    return { success: true };
  },
