...
## 发布 (v3.17.13) — 详情面板标签/颜色/备注保存后界面不刷新（删除标签看似无效）

> 用户反馈：「添加的标签不能删除」。

### 根因
- `DetailPanel` 的 `dayData` 是一个独立 ref，在其 `updateData()` 里被赋值为 `calendarStore.getDayData(date)` 返回的**对象引用**。
- `saveDayData` 内部用 `daysData.value[date] = { ...新对象 }` **替换**了该日期的对象，但 `DetailPanel.dayData` 仍持有**旧对象引用**。
- `TagEditor` 的 `props.tags` 来自 `dayData?.tags`，删除后 `saveDayData` 写入了新对象（已不含该标签），但 `dayData` 引用未变 → `props.tags` 仍含旧标签 → 点 × 看似无反应。
- 对比 `StatusButtons`：它保存后 `emit('update')`，`DetailPanel` 监听 `@update="updateData"` 重读数据，所以状态按钮正常刷新。而 `TagEditor` / `ColorPicker` / `NoteEditor` **都漏了 emit `update`**，导致三者都有同样的「保存后不刷新」隐形 bug（用户只先撞上标签）。

### 修复
| 组件 | 改动 |
|---|---|
| `TagEditor.vue` | 新增 `defineEmits(['update'])`；`addTag`/`removeTag` 改为 `await saveTags(...)`；`saveTags` 保存后 `emit('update')` |
| `ColorPicker.vue` | 新增 `emit('update')`；`setColor` 保存后 `emit('update')` |
| `NoteEditor.vue` | 新增 `emit('update')`；`saveNote` 保存后 `emit('update')` |
| `DetailPanel.vue` | 给 `ColorPicker` / `TagEditor` / `NoteEditor` 补 `@update="updateData"`（对齐 `StatusButtons`） |

- 数据持久化本身没问题（`api.js` 的 `saveDay` 正确写入 `tags`）；纯属 UI 引用陈旧导致不同步。
