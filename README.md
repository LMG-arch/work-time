...
- **根因修复图例错位**：`styles.css` 的 `.legend-item::before`（考勤统计色点伪元素）为全局规则，伪元素成为 grid 匿名第一项占据第 1 列，把生活工作台图例「色点/名称/百分比」挤错位换行；作用域收窄为 `.ratio-legend .legend-item::before`，StatsPage 考勤图例不受影响（回归验证通过）
- 三主题（cosmic/dark/default）实测：占比条/图例/饼图配色随主题令牌联动；新增记账实时刷新
- **版本**：3.17.33 → 3.17.34（versionCode 70 → 71）
...
- 面板内完成状态/颜色/标签/备注标记后，面板保持展开便于连续操作，对应折叠区自动收起；DetailPanel 移除 `marked` emit，收起路径统一由 `selectedDate` 驱动
- 清理未使用导入（useAppStore、internalDate）
- **版本**：3.17.31 → 3.17.32（versionCode 68 → 69）

### v3.16.2 (2026-07-11) — 代码审查修复 M1–M4 + 版本元数据修正
- **M1 面积图周边界对齐**：`WeeklyArea` 周网格线改为按真实周一边界映射（由 `StatsPage` 传入 `weekBoundaries`），不再等距切分；caption「按周」名副其实
- **M2 里程碑连跨**：连续打卡一次跨过多个里程碑（如 6→31 天）时只庆祝最高档，避免重复撒花；基线守卫不变
- **M3 图例色阶对齐**：忙闲图例四档的背景渐变/透明度改为与日格 `.busy-heat` 完全一致（.14/.20/.26/.33），强化「色越深越忙」映射
- **M4 渐变 id 唯一**：`WeeklyArea` 用 Vue `useId()` 生成唯一渐变 id（清理非法字符），替换硬编码 `areaFill`，多实例不串色
- **修复版本元数据 bug**：`android/app/build.gradle` 原硬编码 `versionName "3.13.1"` / `versionCode 28`，导致已发 APK 自报旧版本、更新检查永不收敛（无限弹更新且覆盖安装失败）；现与 `version.json` 对齐为 `3.16.2` / `36`
- **版本**：3.16.1 → 3.16.2（versionCode 35 → 36）

### v3.16.1 (2026-06-29) — Phase 5 收尾增强（里程碑庆祝 + 信息可读性）
- **成长苗里程碑庆祝**：连续打卡跨过 7 / 30 / 100 天时，触发 Phase 4 花瓣庆祝（`window.__celebrate`，premium 守卫在 `signature.js` 内部），同时给成长苗卡片一个轻量脉冲反馈（`milestone-flash`），形成「数据可视化 → 招牌瞬间」闭环；加载历史连胜时仅记录基线、不误触发
- **忙闲热力图例**：日历网格下方新增「忙闲」色阶图例（4 级同款 `color-mix` 色块 + 「色越深越忙」说明），与日格叠色同源，降低色阶理解成本
- **面积图 hover 提示**：`WeeklyArea.vue` 新增鼠标悬停定位最近数据点，浮动 tooltip 显示「X日 · 忙闲 N」并带曲线标记点；指针事件仅在 `.weekly-area` 容器，主题感知、`prefers-reduced-motion` 不影响（纯定位，无动画）
- **版本**：3.16.0 → 3.16.1（versionCode 34 → 35）

### v3.16.0 (2026-06-29) — Phase 5 数据与信息（信息密度升级）
- **忙闲热力月历**：月历日格内叠加忙闲色阶（暖色=忙，基于待办/备注/标签/打卡综合密度），闲置日保持清爽；`data-busy` 强度 0–4 分级，`color-mix` 主题感知、尊重 reduced-motion
- **统计环形月览**：`StatsRing.vue` SVG 环形图按状态占比（上班/休息/出差/请假…）着色，中心显示已记录天数，图例同步
- **每周面积图**：`WeeklyArea.vue` 当月每日忙闲密度的平滑面积曲线，按周划分网格、主题色渐变填充，与日历热力同源
- **连续打卡成长苗**：`GrowthPlant.vue` SVG 植物随连续打卡天数生长（种子→嫩芽→小苗→灌木→树→满树繁花），里程碑换形态，轻摆动画尊重 reduced-motion
- **架构**：三项可视化均为数据展示、常驻可见（不依赖 premium 开关）；颜色全部读取 CSS 变量 / `color-mix`，跟随主题切换；动画均受 `prefers-reduced-motion` 守卫
- **版本**：3.15.3 → 3.16.0（versionCode 33 → 34）
...
- **玻璃颗粒质感**：6 类玻璃卡片叠加 `feTurbulence` 极淡噪声（`opacity 0.035` + `overlay` 混合），在不影响可读性的前提下增加材质感
- **每日诗意 / 节气文案**：日历顶栏新增诗意条，按节气/季节轮换短句（纯本地数据，离线可用）
- **按钮墨水波纹**：全局 `pointerdown` 委托生成主题色涟漪，受 `premium` 总开关与 `prefers-reduced-motion` 守卫
- **列表错落入场**：记录列表 / 标签统计采用递增 `delay` 的错落浮现（独立 `translate` 属性，不与 hover 冲突，尊重 reduced-motion）
- **版本**：3.14.0 → 3.15.0

### v3.14.0 (2026-06-29) — 全主题重构 + 导航动效 + 详情面板修复
- **视觉重构**：15 套主题全量重做，统一为渐变背景 + `color-mix` 派生的毛玻璃卡片，每套主题都有独立色彩性格
- **氛围升级**：新增「更浓辉光层」增强背景光晕；卡片层叠加毛玻璃质感，默认主题「星海绽放」宇宙感更突出
- **动效统一**：全主题接入轻动效——卡片入场 `cardIn`、hover 抬升、基于 View Transitions API 的主题/页面过渡
- **导航体验**：5 个页面切换改为方向感知动画（前进/后退/淡入），底部新增滑动指示块；导航栏散落 emoji 全部替换为 SVG 图标
- **Bug 修复**：详情面板再次点击同一日期无法收起 —— 修复 `DetailPanel` 的 prop 同步守卫，父组件置空时正确收起
- **工程改进**：新增「一键启动器」生产/开发模式脚本，移除旧 `启动上班日历.bat`；`.gitignore` 排除 `.workbuddy/` 工作记忆目录
- **版本**：3.13.1 → 3.14.0
